...
Напиши функцию `createCard(title, text, color)`, которая создаёт DOM-элемент `<div>` с:
- Заголовком `<h3>` с текстом `title`
- Параграфом `<p>` с текстом `text`
- Фоновым цветом `color`
- Классом `"card"`

### Задание 2: Динамический список
Напиши функцию `addItem(listId, text)`, которая:
1. Находит `<ul>` по ID
2. Создаёт `<li>` с текстом
3. Добавляет кнопку "Удалить" внутри `<li>`
4. При клике на "Удалить" — элемент удаляется из списка

### Задание 3: Подсветка элементов
Напиши функцию `highlightAll(selector, color)`, которая находит все элементы по CSS-селектору и устанавливает им фоновый цвет. Вызови её: `highlightAll("p", "lightyellow")`.
...
[⬅️ Предыдущая тема: Асинхронность](async-js.md) | [📚 Оглавление](README.md) | [Следующая тема: События ➡️](events-js.md)
